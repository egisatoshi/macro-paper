...
By combining a not-pattern, we can define a \lstinline{unique} function.
A not-pattern is used to describe that there is no more \lstinline{x} after an occurrence of \lstinline{x}.
...
The \lstinline{processMState} function below implements this process.
...
\lstinline{Something} can handle only wildcards or pattern variables, and is an only matcher that can bind a value to a pattern variable.
...
The \lstinline{processMStates} below implements the whole pattern-matching process while the above \lstinline{processMState} implements a step of this pattern-matching process.
...
\lstinline{processMStates} is recursively called for the rest matching states.
...
\lstinline{processMStates} is recursively called for the result of appending the result of this \lstinline{processMState} and the rest matching states.
...
\todo{Implementation of \lstinline{match-first}}
\new{
...
This section introduces a method for implementing the \lstinline{match-all} macro in Scheme.
...
\subsection{Compiling \lstinline{match-all} to Application of \lstinline{map}}\label{method-map}
...
These pattern-matching results consist of values that are going to bound to the pattern variables returned by \lstinline{extract-pattern-variables}.
...
The Haskell program below shows the definition of \lstinline{Pattern a}, which is a data type for patterns against data whose type is \lstinline{a}.
...
\todo{\lstinline{bpsw-prime?}, \lstinline{stream-iota}, \lstinline{stream-take}}
...
\noindent By doubling nesting the above join-cons pattern, we can define the \lstinline{concat} function.
