...
\todo{definition of Something.}\lstinline{(define Something 'Something)}
...
For that reason, the \texttt{processMStates1} function below is used for \texttt{match-first} instead of \texttt{processMStates}.
...
This limitation comes from the limitation of Haskell that does not allow overlapping data families.
Currently, patterns are defined in the library as data of \lstinline{Pattern a}, which is a data type for patterns against data whose type is \lstinline{a}.
