...
\begin{abstract}
  ...
  The key idea of this method is transformation of a matcher to a function that takes a pattern and a target, and returns lists of triples of a pattern, a matcher, and a target.\todo{more detail}
  % Knowledge
...
These works in 1980s lead to more expressive extensions of pattern matching.
...
\new{
The expressions below match a list ``\texttt{\{1 2 3\}}'' as a list of integers ``\texttt{(list integer)}'' using the pattern ``\texttt{<cons \$x \$ts>}''.
}
...
\noindent
\new{
The \texttt{match-all} expression used in the above program returns a list of the results evaluating the body expression for all the pattern-matching results.
The \texttt{cons} pattern decomposes a list into the head element and the rest.
Therefore, in this case, the evaluation result of this \texttt{match-all} contains just a single element.
Egison uses four kinds of parenthesis for different purpose.
\texttt{()} is used to build a syntax tree or apply a function.
\texttt{<>} is used to apply a pattern constructor to patterns.
\texttt{[]} is used to build a tuple.
\texttt{{}} is used to build a list.
In Egison, a pattern variable begin with ``\texttt{\$}''.
...
The matcher ``\texttt{(list integer)}'' specifies that we do pattern matching the target with the given pattern as a list of integers.
Users can define their own matchers in Egison.
}

\new{
If we change the matcher of the above \texttt{match-all} from \texttt{(list integer)} to \texttt{(multiset integer)}, the evaluation result also changes.
The \texttt{cons} pattern of a multiset is defined to divide a target list into an arbitrary element and the rest.
...
\new{
In addition to \texttt{cons}, the \texttt{join} pattern can be used for the \texttt{list} matcher.
The \texttt{join} pattern divides a target list into two lists, a head part and tail part.
}
...
The expressions below match a list that consists of $n$ zeros ``\texttt{(take n (repeat 0))}'' as a multiset of integers ``\texttt{(multiset integer)}'' with patterns that match a sequential triple ``\texttt{<cons \$x <cons ,(+ x 1) <cons ,(+ x 2) _>>>}''.
...
In Egison, a value pattern that checks equality of data begins with ``\texttt{,}''.
After ``\texttt{,}'',  we can write an arbitrary expression.
...
However, Curry transforms a non-linear pattern to pattern guards\todo{cite}.
Therefore, the time complexity for the same pattern matching is $O(n^3)$ in Curry.
Backtracking is important feature for handling non-linear patterns.
}
...
Egison allows users to define their own matchers.
}
...
\new{
The definition of \texttt{multiset} in Scheme will be explained in Sect.~\ref{method-matcher}.
...
Pattern matching for symbolic mathematical expression was implemented in the symbol manipulation system proposed by McBride~\cite{mcbride1969symbol}, which was developed on top of Lisp.
...
The proposed system supports the \texttt{cons} and the \texttt{join} patterns, \texttt{match-all}, not-patterns, and recursive patterns.
...
\section{Usage of Proposed Macros}\label{usage}

This section introduces the usage of the pattern-matching expressions provided by the proposed Scheme macros.

\subsection{Syntax Design}\label{syntax}

\new{
Here is the formal grammar of the pattern-matching expressions of the proposed Scheme macros.
$M$ denotes an expression.
$p$ denotes a pattern.
$x$ denotes a symbol.
...
\noindent
\begin{align*}
M &::= \texttt{(match-all $M$ $M$ [$p$ $M$] $\cdots$)} \\
&\mid \texttt{(match-first $M$ $M$ [$p$ $M$] $\cdots$)} \\
&\mid \texttt{Something}
\end{align*}
\columnbreak
\begin{align*}
p &::= \texttt{_} \mid \texttt{$x$} \mid \texttt{,$M$} \mid \texttt{($C$ $p$ $\cdots$)} \mid \texttt{'($p$ $\cdots$)} \\
&\mid \texttt{(or $p$ $\cdots$)} \mid \texttt{(and $p$ $\cdots$)} \mid \texttt{(not $p$)} \\
&\mid \texttt{(later $p$)} \\
\end{align*}
\end{multicols}

\todo{segmentation variables and loop patterns}

\new{
We use three kinds of parenthesis.
\texttt{[]} is used to build a tuple.
\texttt{\{\}} is used to build a list.
\texttt{()} is used to build a syntax tree or apply a function and a pattern constructor.
Though we use \texttt{<>} for applying a pattern constructor in Egison, we use \texttt{()} for that in Scheme because we cannot use \texttt{<>} as parenthesis in Scheme.
\todo{refer to R6RS and R7RS for the specification of parenthesis.}
}
...
We do not use the name \texttt{match} to avoid the name conflict with Wright's \texttt{match}.
...
\subsubsection{Wildcard and Pattern Variables}
...
\subsubsection{Inductive Patterns}
...
\todo{The reason of the syntax design}
...
\subsubsection{Logical Patterns}
...
\subsection{Pattern-Matching-Oriented Programming}\label{apps}

\new{
The expressive power of Egison pattern matching is so expressive that a new programming style called \emph{pattern-matching-oriented programming style} arises.
This programming style makes descriptions of mathematical algorithms concise by confining explicit recursions for traversing data inside intuitive patterns.\todo{fix}
...
\subsubsection{Implementation of a SAT Solver}
...
\new{
The \texttt{sat} function takes two argument \texttt{vars}, a list of propositional variable, and \texttt{cnf}, a propositional logic formula in conjunctive normal form.
The tuple consists of \texttt{vars} and \texttt{cnf} is pattern-matched as a tuple of a multiset of integers and a multiset of multisets of integers in the definition of \texttt{sat}.
...
\noindent
\new{
The first match clause (line 5) matches if \texttt{cnf} is empty.
This match clause returns \texttt{\#t} because \texttt{cnf} is satisfiable in this case.
The second match clause (line 7) matches if \texttt{cnf} contains an empty clause.
This match clause returns \texttt{\#f} because \texttt{cnf} is unsatisfiable in this case.
The third match clause (lines 9 and 10) matches if \texttt{cnf} contains a clause that consists of a single literal \texttt{l}.
In this match clause, we assign \texttt{l} true at once.
The fourth match clause (lines 12 and 13) matches when there is a propositional variable \texttt{v} that appears only positively in \texttt{cnf}.
In this match clause, we assign \texttt{v} true at once.
The fifth match clause (lines 15 and 16) matches when there is a propositional variable \texttt{v} that appears only negatively in \texttt{cnf}.
In this match clause, we assign \texttt{v} false at once.
...
\subsubsection{Redefinitions of Basic List Processing Functions}

\todo{more detail explanation}
By combining a not-pattern, we can define a \texttt{unique} function.
...
In \texttt{processMStates} of \texttt{stream-egison.scm}, the traversal of an infinitely large search tree is implemented that is explained in Sect.~5.2 of the original Egison paper~\cite{egi2018aplas}.
Appendix~\ref{pm-inf} demonstrates pattern matching that uses the macros provided by \texttt{stream-egison.scm}.

\todo{Implementation of \texttt{match-first}}
\new{
This distinct implementation of \texttt{processMStates} is not necessary for the Egison language that adopt a lazy evaluation strategy.
}
...
The three tricks that are explained in Sect.~\ref{method-matcher}, Sect.~\ref{method-map}, and Sect.~\ref{method-val-pat} enable this implementation.
...
The first match clause (line 5) describes the pattern-matching algorithm for the \texttt{nil} pattern that matches with an empty list.
...
The second match clause (lines 6-9) describes a pattern-matching algorithm for the \texttt{cons} pattern.
...
\noindent
\new{
\texttt{define-macro} in the above program is a traditional non-hygienic macro similar to \texttt{defmacro} of Common Lisp~\cite{gaucheTM}.
The \texttt{match-all} macro requires to call the complex Scheme procedures such as \texttt{extract-pattern-variables} for expanding the macro.
Therefore, defining the macro just by using \texttt{syntax-rules}, which is ...\todo{find a good verb} in R7RS\todo{cite} is difficult.
By using \texttt{syntax-case} and \texttt{syntax->datum} in R6RS, we can define the above macro relatively easily.
}
...
Thus, by separating the extraction of pattern variables and the calculation of pattern-matching results, we can compile the large pattern matching procedure of Egison.
...
First, we can improve the performance of the \texttt{Multiset} implementation in Sect.~\ref{method-matcher} using this method.
...
Therefore, by adding a match clause for a \texttt{join} pattern whose first argument is a wildcard as follows, we can improve the efficiency of pattern matching for a list.
...
\noindent
\new{
I also made a comparison with the functional description of the same program.
}
...
Haskell does not allow to add new data constructors to a data type defined in a different file.\todo{cite: data family overlapping}
...
\todo{explanation of the following code}
...
%\begin{acks}
%  I thank Yuichi Nishiwaki for his advice ...
%  I thank Yuito Murase for implementing Common Lisp macros.
%  I thank Mayuko Kori for helping me for implementing Template Haskell implementation of the proposed Scheme macros.
%\end{acks}
...
\todo{\texttt{bpsw-prime?}, \texttt{stream-iota}, \texttt{stream-take}}
...
\section{Basic List Processing Functions in Pattern-Matching-Oriented Style}\label{pmo-list}
