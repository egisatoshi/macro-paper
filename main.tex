...
\begin{abstract}
  ...
  % Knowledge
  This paper also demonstrates the expressiveness of this pattern-matching system by showing redefinitions of the basic list processing functions such as \lstinline{map}, \lstinline{concat}, or \lstinline{unique}, and implementation of a SAT solver as a more practical mathematical algorithm.
...
\new{
The expressions below match a list ``\lstinline|{1 2 3}|'' as a list of integers ``\lstinline{(list integer)}'' using the pattern ``\lstinline{<cons $x $ts>}''.
}
...
The matcher ``\lstinline{(list integer)}'' specifies that we do pattern matching the target with the given pattern as a list of integers.
...
The expressions below match a list that consists of $n$ zeros ``\lstinline{(take n (repeat 0))}'' as a multiset of integers ``\lstinline{(multiset integer)}'' with patterns that match a sequential triple ``\lstinline{<cons $x <cons ,(+ x 1) <cons ,(+ x 2) _>>>}''.
...
Pattern guards are applied after pattern matching succeeds.
...
Therefore, it is safe to replace braces and square brackets to parenthesis when we export the Scheme program in this paper to other Scheme implementation.
}
}
...
We do not use the name \texttt{match} to avoid the name conflict with Wright's \texttt{match}~\cite{wright1993pattern} because Wright's \texttt{match} plays fundamental role for defining a matcher, which will be explained in Sect.~\ref{method-matcher}.
...
\new{
The use of ``\lstinline{,}'' to notate a value pattern is an analogy of the use of ``\lstinline{,}'' for \lstinline{unquote}.
...
\new{
``\lstinline{'}'' distinguishes a tuple pattern from a inductive pattern.
...
We can see the full implementation of \texttt{processMState} in \texttt{egison.scm} in the GitHub repository of the proposed macros~\cite{egisonScheme}.
...
In \texttt{processMStates} of \texttt{stream-egison.scm}~\cite{egisonScheme}, the traversal of an infinitely large search tree is implemented that is explained in Sect.~5.2 of the original Egison paper~\cite{egi2018aplas}.
...
Therefore, defining the macro just by using \texttt{syntax-rules}, which is supported in R7RS~\cite{shinn2013revised} is difficult.
By using \texttt{syntax-case} and \texttt{syntax->datum} in R6RS~\cite{sperber2009revised}, we can define the above macro relatively easily.
}

The program below defines \texttt{gen-match-results}.
\texttt{gen-match-results} is a macro that calls the \texttt{processMStates} function introduced in Sect.~\ref{algorithm}.
An initial matching state ``\lstinline|(MState {[p M t]} {})|'' is created from ``\texttt{p}'', ``\texttt{M}'', and ``\texttt{t}''.
...
\texttt{match-all} provided by \texttt{stream-egison.scm} supports pattern matching with infinitely many results.
