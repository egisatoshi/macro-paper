...
Pattern matching is an important feature of programming languages for data abstraction.
Pattern matching allows us to replace many verbose function applications (such as \texttt{car} and \texttt{cdr}) for decomposing data to an intuitive pattern.
The most of modern functional programming languages support pattern matching.
Many Scheme implementation also support pattern matching for algebraic data types~\cite{gauchePM} though Scheme does not contain it in its specification, R7RS~\cite{shinn2013revised}.

\todo{why pattern matching for algebraic data types is not enough?}
...
The pattern-matching algorithm proposed by them is not as simple as the earlier pattern-matching extensions that can be compiled into simple conditional branches; they implemented their proposal only in an interpreter of the Egison programming language.
As a result, a method for importing the Egison pattern matching system into the other functional programming languages was not obvious.

This paper presents a method for integrating this Egison pattern matching to a dynamically typed functional programming language by introducing Scheme macros for importing Egison pattern matching.
...
However, pattern matching for algebraic data types designed by Wright~\cite{wright1993pattern} is implemented in most of well-known Scheme implementations such as Gauche~\cite{gauchePM}, Guile~\cite{guilePM}, and Chicken~\cite{chickenPM}.
...
The \texttt{match-first} has similar to Wright's \texttt{match} expression and it evaluate the body of the first match clause where the pattern matches with the target.
The \texttt{match-first} expression can take multiple match clause.

This pattern-matching library provides several pattern constructs.
The following subsection explains each of them.
...
When the pattern is a list, the first element is a pattern constructor.
For example, \texttt{join} and \texttt{cons} that appear in ``\texttt{(join _ (cons x _))}'' are pattern constructors.
...
\noindent An and-pattern matches if all the argument patterns match the target.

\begin{lstlisting}[language=egison]
(match-all '(1 2 3) (List Integer) [(cons (and ,1 x) _) x]) ; (1)
\end{lstlisting}

\noindent A not-pattern matches if the argument pattern does not match the target.
...
Let me show a specific example.
\todo{...}
...
I also omit the implementation of and-patterns, or-patterns, not-patterns, and later patterns.
We can add implement These patterns by adding match clauses for them in \texttt{processMState}.
We can see the full implementation in \texttt{egison.scm} of~\cite{egisonScheme}.
...
\todo{explain this matcher clause}
...
The proposed method allows us to implement very expressive pattern matching in a short program; the macros and the pattern-matching procedure is implemented in less than 150 lines of code (\texttt{egison.scm} of~\cite{egisonScheme}).
This is a good proof of extensibility of Scheme.\todo{fix}
...
This pattern matching library will help research of pattern matching by providing a easy method for extending it.
